...
#define START_HOOK(EVENT, CREATURE) \
    if (!IsEnabled())\
        return;\
    auto entry_key = EntryKey<Hooks::CreatureEvents>(EVENT, CREATURE->GetEntry());\
    auto unique_key = UniqueObjectKey<Hooks::CreatureEvents>(EVENT, CREATURE->GET_GUID(), CREATURE->GetInstanceId());\
